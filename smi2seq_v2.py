...
class SMILES2Sequence:
    ...
    def convert(
        ...
        if staple_records:
            ...
            linker_smiles = self._extract_linker_smiles(clean, staple_records)
    ...
    def _extract_linker_smiles(
        self, smiles: str, records: List[Dict[str, int]]
    ) -> Optional[str]:
        if not records:
            return None
        ...
        frag = rw.GetMol()
        fragments = Chem.GetMolFrags(frag, asMols=True, sanitizeFrags=False)
        linker_mol = None
        for candidate in fragments:
            if any(atom.GetAtomicNum() == 0 and atom.GetAtomMapNum() != 0 for atom in candidate.GetAtoms()):
                linker_mol = candidate
                break
        if linker_mol is None:
            return None
        Chem.SanitizeMol(linker_mol)
        return Chem.MolToSmiles(linker_mol, isomericSmiles=True)
